...
async function assertDomContracts() {
  ...
  assert(html.includes('id="btn-howto-back"'), "howto back button exists");
  assert(html.includes('id="btn-game-back"'), "game back button exists");
...
for (const id of ["easy", "normal", "hard", "expert"]) {
  ...
  assert(Array.isArray(puzzle.hints) && puzzle.hints.length >= 2, `${id} has multiple answer hints`);
...
assert(easy.hints.length >= 2, "easy has at least two answer hints");
assert(
  easy.hints.every((h) => h.stage === "fact"),
  "all hints are answer facts from clue numbers",
);
assert(
  easy.hints.every((h) => /단서\s+\d+번/.test(h.text) && h.text.includes("을 통해")),
  "hints only state which clue numbers yield the answer",
);
assert(
  easy.hints.every(
    (h) =>
      h.meta?.clueIndices?.every(
...
assert(
  easy.hints[0].meta?.val === easy.answer[easy.hints[0].meta.cat]?.[easy.hints[0].meta.houseIndex],
  "hint fact matches answer placement",
);
assert(
  easy.hints[0].text.includes("번") && !easy.hints[0].text.includes("2번과 5번"),
...
assert(hard.hints.length >= 3, "hard has at least three answer hints");
...
assert(game.revealNextHint().stage === "fact", "first reveal is clue-to-answer hint");
assert(game.revealNextHint().stage === "fact", "second reveal is clue-to-answer hint");
assert(game.getRevealedHints().length === 2, "two hints revealed");
game.toggleClueRead(runtimePuzzle.clues[0].id);
assert(!game.isClueRead(runtimePuzzle.clues[0].id), "clue read toggles off");
game.resetPlacement({ keepTimer: true });
assert(game.getRevealedHints().length === 2, "resetPlacement keeps revealed hints");
game.loadPuzzle(runtimePuzzle);
